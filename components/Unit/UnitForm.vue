...
export default {
  ...
  props: {
    redirectUrl: { type: String, default: '/setting/units' },
    cancelUrl: { type: String, default: '/setting/units' },
  ...
  mounted() {
    this.resetForm()
    this.formTitle = this.isEdit ? 'Edit Unit' : 'Add Unit'
  ...
  methods: {
    ...
    submitForm() {
      ...
      const endpoint = this.isEdit ? 'unit/' + this.data.uuid : 'unit'
      this.$axios
        .post(endpoint, data)
        .then((response) => {
          this.$buefy.snackbar.open({
